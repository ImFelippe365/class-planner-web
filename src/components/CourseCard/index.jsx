...
export default function CourseCard ({courseGrade, courseName}){
    const [isOpen, setIsOpen] = useState(false)

    return (
        <div className={`${courseGrade == "Técnico Integrado" ? "bg-ultra-violet-transparent": "bg-coffee-transparent"} rounded-xl w-56 p-4 items-center`}>
            <div className="flex flex-row justify-between items-center">
                <GraduationCap width={29} height={29} color={`${courseGrade == "Técnico Integrado" ? "#52489C" : "#6D4C3D"}`}/>

                <div className="relative">
                    <button className="outline-none flex" onClick={() => setIsOpen((prev) => !prev)}>
                        <MoreVertical width={18} height={18} color={`${courseGrade == "Técnico Integrado" ? "#52489C" : "#6D4C3D"}`} className="self-center"/>
                    </button>
            ...
            <div className={`${courseGrade == "Técnico Integrado" ? "text-ultra-violet" : "text-coffee"} mt-3 h-20`}>
